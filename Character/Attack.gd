...
func state_input(event : InputEvent):
	if(event.is_action_pressed("attack")):
		timer.start()  # a buffer, the time window
		


func _on_animation_tree_animation_finished(anim_name):
	if(anim_name == attack1_name):
		if(timer.is_stopped()): # it'll give the player a time window to do a chaied attack within the time given, then itll return to old state 
